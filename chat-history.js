...
router.delete('/chat-history/:id', async (req, res) => {
  try {
    ...
    await pool.execute('DELETE FROM pesan_chat WHERE id = ?', [id]);
...
router.post('/chat/save', async (req, res) => {
  ...
  try {
    ...
    const [chatResult] = await connection.execute(`
      INSERT INTO pesan_chat (user_email, judul)
      VALUES (?, ?)
    `, [userEmail, chatTitle]);
    ...
    for (const message of messages) {
      ...
      await connection.execute(`
        INSERT INTO pesan_chat_detail (riwayat_chat_id, peran, konten, sumber)
        VALUES (?, ?, ?, ?)
...
router.put('/chat/update', async (req, res) => {
  ...
  try {
    ...
    const [existingMessages] = await connection.execute(
      'SELECT COUNT(*) as count FROM pesan_chat_detail WHERE riwayat_chat_id = ?',
      [dbId]
    );
    ...
    for (const message of newMessages) {
      ...
      if (io) {
        const [insertResult] = await connection.execute(
          'SELECT id FROM pesan_chat_detail WHERE riwayat_chat_id = ? ORDER BY id DESC LIMIT 1',
          [dbId]
        );
    ...
    await connection.execute(
      'UPDATE pesan_chat SET diperbarui_pada = NOW() WHERE id = ?',
      [dbId]
    );
...
router.get('/chat/history/:email', async (req, res) => {
  try {
    ...
    if (!email || email === 'null' || email === 'undefined') {
      console.log('❌ Invalid email, returning empty array');
    ...
    console.log(`✅ Found ${chatRows.length} chat sessions for ${email}`);
    ...
    for (const chat of chatRows) {
      ...
      console.log(`  Chat #${chat.id}: "${chat.judul}" - ${messageRows.length} messages`);
    ...
    console.log('📤 Sending', chatHistory.length, 'chat sessions to frontend');

    // Return direct array format for compatibility
